...
<?php } ?>
		  ...
		  <div class="form-group row">
			...
			<input type="hidden" name="sname" id="sid" value="<?php echo $res[0];?>">
			  <input type="button" class="btn btn-primary btn-md" id="<?php echo $ids; ?>" onclick="employer_deduction_update(this.id)" value="Update">
			</div>
		  </div>
	</form>
    </div>
...
function employer_deduction_update(v)
{
	...
	$.ajax({
		type:'GET',
		data:"id="+v, data,
		url:'deduction/employer_deduction_update.php',
		success:function(data)
		{
			 if(data==1)
			{
