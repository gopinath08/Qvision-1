...
$voucher=mysql_fetch_array(mysql_query("SELECT code,date,voucher_category_code,voucher_purpose_code,slip_no,reference_voucher,member_no, name,branch_code,reference_no,
ledger_code,amount,bank_code,cheque_no,cheque_date,description,narration,status FROM voucher 
WHERE code='$voucher_code'"));
$vou_cat_code=$voucher['voucher_category_code'];
$vou_pur_code=$voucher['voucher_purpose_code'];
$voc_cat_name=mysql_fetch_array(mysql_query("select name from voucher_category where code='$vou_cat_code'"));
$voc_purp_name=mysql_fetch_array(mysql_query("select name from voucher_purpose where code='$vou_pur_code'"));
$date=$voucher['date'];
...
<?php
$v_sql=mysql_query("SELECT ledger_code,reference,amount,type,narration,status FROM voucher_detail WHERE voucher_code='$voucher_code'");
...
<tr>	
	...
	<?php $led_name=mysql_fetch_array(mysql_query("SELECT name FROM ledger WHERE code='$ledger'"));?>
	<option value="<?php echo $ledger; ?>"><?php echo $ledger.'-'.$led_name['name']; ?></option>
	<?PHP
	$led_sql="SELECT code,name FROM ledger WHERE code<>'$ledger' ";
	$led_row=mysql_query($led_sql);
	while($led_res=mysql_fetch_array($led_row))
	{
	?>
...
function voucher_update()
{
	...
	$.get('new_voucher/Update/voucher_update.php',data);
	new_voucher();
}
</script>
<script>
