...
class voucher_process
{
	...
	{
		echo $category_code=$this->voucher_category_code;
		echo $purpose_code=$this->voucher_purpose_code;
		$code=$this->code;
		$status=$this->status;
		...
		if($category_code=='CAT-001')
		{
			...
			for($k=0;$k<$length;$k++)		
			{
				$voucher_detail=mysql_query("INSERT INTO voucher_detail(voucher_code,sequence,ledger_code, reference,amount,type,narration, status, created_by, created_on) VALUES('$code','1','$ledgers_code[$k]','$reference','$ledgers_amount[$k]','$ledgers_type[$k]','$AEnar','1','$created_by',NOW())");
			}
		}
		if($category_code=='CAT-002')
		{
			$reference=$this->reference_no;	
