...
require '../connect.php';
		...
		if($payroll_update)
		{
			...
			$inndate1=$con->query("SELECT emp_code as emp_no FROM staff_master WHERE status=1 and prefix_code in ('QSPL','')");
			...
			while($att_result_query=$inndate1->fetch(PDO::FETCH_ASSOC))
			{
				...
				$saturday_count=0;
					...
					for($i=0;$i<sizeof($dateMonthYearArr);$i++)
					{
						...
						$dayquery=$con->query("SELECT COUNT(*) FROM qds_attendance_view WHERE emp_no='$emp_no' and in_log_date='$date'");
				...
				{					
					...
					while($deduct_data = $deduct_sql->fetch(PDO::FETCH_ASSOC))
					{
						...
						{
								$percentage;
								$cal_amount = ($salary_amount*$percentage/100);

								try
								{
									$data = array($date,$month,$year,$emp_code,$emp_name,$deduction,$cal_amount,1,1,$date);
