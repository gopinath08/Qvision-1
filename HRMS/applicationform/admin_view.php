	...
	<tr>
	 ...
	 <tr>
		<?php $sql=$con->query("SELECT * FROM `emp_qualification` where emp_id='$id'");  while($rows = $sql->fetch(PDO::FETCH_ASSOC)) { ?>
		 ...
		 <td><iframe src="https://docs.google.com/viewer?url=https://Qvision/Qvision/Qvision/uploads/<?php echo $id; ?>/<?php echo $rows['attachment']; ?>">a</iframe></td>
		<?php } ?>
 	 </tr>
 	 <tr>
...
function mail_Send(id)
{
	$.ajax({
	type:"POST",
	url:"Qvision/applicationform/mail_send.php?id="+id,
	success:function(data)
	{
		if(data==0)
		{
