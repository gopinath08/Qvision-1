...
$sql=$con->query("SELECT * FROM resource_form_detail s left join designation_master d on s.position=d.id join source_master sm on s.source=sm.id where s.status='$resourceid'");
$fet=$sql->fetch();
?>
...
function schedule_insert()
{
	...
	$.ajax({
		...
		type:"GET",
		data: data,
		url:"/Qvision/Qvision/resource/resource_form/schedule_insert.php",
